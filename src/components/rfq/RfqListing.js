    ...
    const buttonFormatter = (props) => {
        ...
        return (
            <>
                ...
                {(status !== APPROVED && status !== UNDER_APPROVAL && status !== CANCELLED) && rowData?.IsShowCancelIcon && <button title='Cancel' className="CancelIcon mr-1" type={'button'} onClick={() => cancelItem(cellValue)} />}
            </>
        )
    };
