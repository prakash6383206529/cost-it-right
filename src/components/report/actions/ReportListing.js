...
export function getReportListing(index, take, isPagination, data, callback) {
    ...
    return (dispatch) => {
        if (isPagination === true) {
            ...
            const request = axios.get(`${API.getReportListing}?${queryParams}${queryParamsSecond}${departmentQueryParams}`, config());
            request.then((response) => {
                if (response.data.Result || response.status === 204) {
                    dispatch({
                        type: GET_REPORT_LIST,
...
export function getCostingReport(skip, take, isPagination, obj, isLastWeek, isCallApi, callback) {
    ...
    return (dispatch) => {
        if (isCallApi === true) {
            ...
            const queryParams = `applyPagination=${isPagination}&skip=${skip}&take=${take}&CostingHead=${obj.CostingHead ? obj.CostingHead : ""}&NCCPartQuantity=${obj.NCCPartQuantity ? obj.NCCPartQuantity : ""}&IsRegularized=${obj.IsRegularized ? obj.IsRegularized : ""}`
