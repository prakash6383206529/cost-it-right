...
import { sendReminderForQuotation, getQuotationDetailsList, getMultipleCostingDetails, setQuotationIdForRFQ, getQuotationList } from './actions/rfq';
                                ...
                                <div className={`ag-grid-wrapper ${(props?.isDataInMaster && noData) ? 'master-approval-overlay' : ''} ${(rowData && rowData?.length <= 0) || noData ? 'overlay-contain' : ''}`}>
                                    <div className={`ag-theme-material ${(loader && !props.isMasterSummaryDrawer) && "max-loader-height"}`}>
                                        ...
                                        >
                                            ...
                                            {initialConfiguration?.IsNFRConfigured && <AgGridColumn cellClass={cellClass} field="NfrNo" headerName='Customer RFQ No.' cellRenderer={seperateHyphenFormatter}></AgGridColumn>}
