...
function TabAssemblyTechnology(props) {
  ...
  const { ComponentItemData, ComponentItemDiscountData, ErrorObjRMCC, CostingEffectiveDate, getAssemBOPCharge, SurfaceTabData, OverheadProfitTabData, PackageAndFreightTabData, ToolTabData, DiscountCostData, checkIsDataChange } = useSelector(state => state.costing)
  ...
  */
  const setRMCost = (rmGrid, params, item) => {
    const arr = setRMCostInDataList(rmGrid, params, subAssemblyTechnologyArray, item)
    if (subAssemblyTechnologyArray[0].PartNumber === params.PartNumber && subAssemblyTechnologyArray[0].BOMLevel === params.BOMLevel) {
  ...
  */
  const setRMMasterBatchCost = (rmGrid, MasterBatchObj, params) => {
    let arr = setRMBatchCostInDataList(rmGrid, MasterBatchObj, params, subAssemblyTechnologyArray)
    dispatch(setRMCCData(arr, () => { }))
  }
  ...
  */
  ...
  const setBOPCost = (bopGrid, params, item) => {
    const arr = setBOPCostInDataList(bopGrid, params, subAssemblyTechnologyArray, item)
    if (subAssemblyTechnologyArray[0].PartNumber === params.PartNumber && subAssemblyTechnologyArray[0].BOMLevel === params.BOMLevel) {
      dispatch(setRMCCData(arr, () => { }))
    } else {
  ...
  const setBOPHandlingCost = (bopGrid, BOPHandlingFields, params) => {
    let arr = setBOPHandlingCostInDataList(bopGrid, BOPHandlingFields, params, subAssemblyTechnologyArray)
    dispatch(setRMCCData(arr, () => { }))
  }
  ...
  */
  ...
  const setProcessCost = (conversionGrid, params, item) => {
    let arr = setProcessCostInDataList(conversionGrid, params, subAssemblyTechnologyArray, item)
    if (subAssemblyTechnologyArray[0].PartNumber === params.PartNumber && subAssemblyTechnologyArray[0].BOMLevel === params.BOMLevel) {
      dispatch(setRMCCData(arr, () => { }))
    } else {
  ...
  */
  ...
  const setOperationCost = (operationGrid, params, item) => {
    let arr = setOperationCostInDataList(operationGrid, params, subAssemblyTechnologyArray, item)
    if (subAssemblyTechnologyArray[0].PartNumber === params.PartNumber && subAssemblyTechnologyArray[0].BOMLevel === params.BOMLevel) {
      dispatch(setRMCCData(arr, () => { }))
    } else {
      const arr1 = assemblyCalculation(arr, 'CC') // THIS FUNCTION IS FOR ASSEMBLY CALCULATION
  ...
  const setOtherOperationCost = (otherOperationGrid, params, item) => {
    let arr = setOtherOperationCostInDataList(otherOperationGrid, params, subAssemblyTechnologyArray, item)
    if (subAssemblyTechnologyArray[0].PartNumber === params.PartNumber && subAssemblyTechnologyArray[0].BOMLevel === params.BOMLevel) {
      dispatch(setRMCCData(arr, () => { }))
    } else {
      const arr1 = assemblyCalculation(arr, 'CC') // THIS FUNCTION IS FOR ASSEMBLY CALCULATION
  ...
  const setToolCost = (toolGrid, params) => {
    let arr = setToolCostInDataList(toolGrid, params, subAssemblyTechnologyArray)
    dispatch(setRMCCData(arr, () => { }))
  }
  ...
  */
  const setPartDetails = (BOMLevel, PartNumber, Data, item) => {
    if (item.PartType === 'Component') {
      let arr = formatData(BOMLevel, PartNumber, Data, subAssemblyTechnologyArray, item)
      dispatch(setRMCCData(arr, () => { }))
  ...
  */
  const formatData = (BOMLevel, PartNumber, Data, subAssemblyTechnologyArray, item) => {
  ...
  */
  const setAssemblyToolCost = (ToolGrid, params) => {
    let arr = setAssemblyToolCostInDataList(ToolGrid, params, subAssemblyTechnologyArray)
    dispatch(setRMCCData(arr, () => { }))
  }
