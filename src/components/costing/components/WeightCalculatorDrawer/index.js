            ...
            {Number(technology) !== Number(RUBBER) && Number(technology) !== Number(Ferrous_Casting) && (showPaperCorrugatedBox && Number(technology) !== Number(CORRUGATEDBOX)) &&
              <Row className="mt-4 mb-4 pb-2">
                <Col md="12 d-flex weight-calculator-headings">
                  ...
                  <div className="d-inline-block "><span className="grey-text d-block">RM Rate ({sourceCurrencyFormatter(currencySource?.label)}/{rmRowData?.UOMSymbol}):</span><span className="text-dark-blue">{`${rmRowData.RMRate !== undefined ? checkForDecimalAndNull(rmRowData.RMRate, getConfigurationKey().NoOfDecimalForPrice) : ''}`}</span></div>
