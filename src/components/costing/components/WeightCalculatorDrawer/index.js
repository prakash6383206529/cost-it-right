            ...
            {Number(technology) !== Number(RUBBER) && Number(technology) !== Number(Ferrous_Casting) &&
              <Row className="mt-4 mb-4 pb-2">
                <Col md="12 d-flex weight-calculator-headings">
                  ...
                  <div className="d-inline-block "><span className="grey-text d-block">{`${Number(technology) === Number(MACHINING) ? `Scrap Rate Per Scrap UOM(${getConfigurationKey()?.BaseCurrency}/${rmRowData.ScrapUnitOfMeasurement})` : `Scrap Rate(${getConfigurationKey()?.BaseCurrency})`}:`}</span><span className="text-dark-blue">{`${rmRowData.ScrapRate !== undefined ? Number(technology) === Number(MACHINING) ? rmRowData.ScrapRatePerScrapUOMConversion : rmRowData.ScrapRate : ''}`}</span></div>
