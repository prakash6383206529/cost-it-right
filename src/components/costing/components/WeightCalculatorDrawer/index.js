            ...
            {Number(technology) !== Number(RUBBER) && Number(technology) !== Number(Ferrous_Casting) &&
              <Row className="mt-4 mb-4 pb-2">
                <Col md="12 d-flex weight-calculator-headings">
                  ...
                  {Number(technology) === Number(MACHINING) && <div className="d-inline-block "><span className="grey-text d-block">Scrap Rate UOM:</span><span className="text-dark-blue">{rmRowData.ScrapUnitOfMeasurement}</span></div>}
                  <div className="d-inline-block "><span className="grey-text d-block">{`${Number(technology) === Number(MACHINING) ? 'Scrap Rate Per Scrap UOM' : 'Scrap Rate'}(INR):`}</span><span className="text-dark-blue">{`${rmRowData.ScrapRate !== undefined ? Number(technology) === Number(MACHINING) ? rmRowData.ScrapRatePerScrapUOM : rmRowData.ScrapRate : ''}`}</span></div>
