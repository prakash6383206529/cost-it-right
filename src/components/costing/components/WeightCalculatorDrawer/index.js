            ...
            {Number(technology) !== Number(RUBBER) && Number(technology) !== Number(Ferrous_Casting) &&
              <Row className="mt-4 mb-4 pb-2">
                <Col md="12 d-flex weight-calculator-headings">
                  ...
                  {Number(technology) === Number(MACHINING) && <div className="d-inline-block "><span className="grey-text d-block">{`Scrap Rate Per Scrap UOM(${getConfigurationKey()?.BaseCurrency}/${rmRowData.ScrapUnitOfMeasurement}):`}</span><span className="text-dark-blue">{`${rmRowData.ScrapRate !== undefined ? Number(technology) === Number(MACHINING) ? (rmRowData.ScrapRatePerScrapUOMConversion ? rmRowData.ScrapRatePerScrapUOMConversion : rmRowData.ScrapRatePerScrapUOM) : rmRowData.ScrapRate : ''}`}</span></div>}
                  <div className="d-inline-block "><span className="grey-text d-block">{Number(technology) === Number(FORGING) ? 'Forging Scrap' : 'Scrap'} Rate({getConfigurationKey()?.BaseCurrency}/{rmRowData.UOM}):</span><span className="text-dark-blue">{`${rmRowData.ScrapRate !== undefined ? rmRowData.ScrapRate : ''}`}</span></div>
                  {Number(technology) === Number(FORGING) && <div className="d-inline-block "><span className="grey-text d-block">Machining Scrap Rate({getConfigurationKey()?.BaseCurrency}/{rmRowData.UOM}):</span><span className="text-dark-blue">{`${rmRowData.MachiningScrapRate ? rmRowData.MachiningScrapRate : 0}`}</span></div>}
