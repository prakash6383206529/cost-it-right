    ...
    if (!isAssemblyTechnology) {
      ...
      const totalProcessCost = arr && arr[0]?.CostingPartDetails?.CostingProcessCostResponse?.reduce((acc, item) => {
        if (!(item?.IsChild)) {
          ...
          if (item?.CostingPartDetails?.ProcessTechnologyId !== costingTechnologyId) {
  ...
  */
  const saveData = () => {
    ...
    tempArr && tempArr.map((item) => {
      if (!(item?.IsChild)) {
        // Calculate total process cost (for all technologies)
        NetProcessCost = checkForNull(NetProcessCost) + checkForNull(item?.ProcessCost)
        // Calculate process cost only for different technologies
        if (item?.CostingPartDetails?.ProcessTechnologyId !== TechnologyId) {
      ...
      if (isAssemblyTechnology) {
        ...
        tempsubAssemblyTechnologyArray[0].CostingPartDetails.NetCCForOtherTechnologyCostCost = NetCCForOtherTechnologyCost
        // Update overhead and profit costs
