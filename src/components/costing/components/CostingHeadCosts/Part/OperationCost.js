              ...
              <div>
                ...
                {(!CostingViewMode && !IsLocked && !isDisable) &&
                  <Button
                    ...
                    buttonName={"OPER"}
                  />}
                {!CostingViewMode && !isDisable && <TooltipCustom tooltipClass="operation-defination" customClass="mt-2 mr-2" id={`operation-defination`} width="400px" tooltipText={"An operation is a detailed step within the manufacturing process involved in creating a part. It often includes tasks where human effort is added, like putting parts together, checking quality, or doing other hands-on work to complete the manufacturing process."} />}
              </div>
            </Col>
          </Row>
          <Row>
                ...
                <tbody >
                  {
                    ...
                    gridData.map((item, index) => {
                      return (
                        editIndex === index ?
                          <tr key={index}>
                            <td className='text-overflow'><span title={item.OperationName + index} draggable={(CostingViewMode || IsLocked || isDisable) ? false : true} >{item.OperationName}</span> </td>
                          ...
                          :
                          <tr key={index}>
                            <td className='text-overflow'><span title={item.OperationName + index} draggable={(CostingViewMode || IsLocked || isDisable) ? false : true} onClick={() => setOpenOperationForm({ isOpen: true, id: item.OperationId })} className='link'>{item.OperationName}</span> </td>
                            <td>{item.OperationCode}</td>
                            <td>{item.UOM}</td>
                            <td>{item.Rate}</td>
                            ...
                            <td>
                              <div className='action-btn-wrapper'>
                                {(!CostingViewMode && !IsLocked && !isDisable) && <button title='Edit' id={`operationCost_edit${index}`} className="Edit mb-0 align-middle" type={'button'} onClick={() => editItem(index)} />}
                                {(!CostingViewMode && !IsLocked && !isDisable) && <button title='Delete' id={`operationCost_delete${index}`} className="Delete mb-0 align-middle" type={'button'} onClick={() => deleteItem(index, item.OperationId)} />}
