...
export function getInventoryDataByHeads(data, callback) {
  return (dispatch) => {
    //dispatch({ type: API_REQUEST });
    const request = axios.get(`${API.getInventoryDataByHeads}?vendorId=${data?.vendorId}&isVendor=${data?.isVendor}&plantId=${data?.plantId}&effectiveDate=${data?.effectiveDate}&customerId=${data.customerId}`, config());
    request
      .then((response) => {
        callback(response)
      })
