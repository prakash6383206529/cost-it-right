...
function RMIndexationSimulationListing(props) {
    ...
    useEffect(() => {
        if (rmIndexationSimulationList?.length > 0) {
            setTotalRecordCount(rmIndexationSimulationList[0].TotalRecordCount)
        }
        else {
            setNoData(false)
        }

    }, [rmIndexationSimulationList])
    ...
    */
    const getDataList = (dataObj, skip = 0, take = 10, isPagination = true, isReset = false) => {
        ...
        if (String(master) === RMDOMESTIC) {
            ...
            filterData.RawMaterialEntryType = props?.FromExchangeRate ? Number(ENTRY_TYPE_IMPORT) : null
    ...
    const onSearch = () => {
        ...
        getDataList(floatingFilterData, 0, globalTakes, true, true)
    }
    ...
    const resetState = () => {
        ...
        setPageNoNew(1)
        dispatch(updateCurrentRowIndex(10))
        getDataList(floatingFilterData, 0, 10, true, true)
        dispatch(setSelectedRowForPagination([]))
    ...
    */
    ...
    const onExcelDownload = () => {
        ...
        if (tempArr?.length > 0) {
            ...
            getDataList(floatingFilterData, 0, defaultPageSize, false) // FOR EXCEL DOWNLOAD OF COMPLETE DATA
        }
                    ...
                    {(loader && !props.isMasterSummaryDrawer) ? <LoaderCustom customClass="simulation-Loader" /> :
                        <>
                            ...
                            <Row>
                                ...
                                <Col>
                                    <div className={`ag-grid-wrapper ${(props?.isDataInMaster && !noData) ? 'master-approval-overlay' : ''} ${(rmIndexationSimulationList && rmIndexationSimulationList?.length <= 0) || noData ? 'overlay-contain' : ''}`}>
...
export default RMIndexationSimulationListing;
