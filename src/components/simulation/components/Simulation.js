...
import { getListingForSimulationCombined, getSelectListOfMasters, setMasterForSimulation, setTechnologyForSimulation, setVendorForSimulation, getTokenSelectListAPI, setTokenCheckBoxValue } from '../actions/Simulation';
...
function Simulation(props) {
    ...
    const { selectedMasterForSimulation, selectedTechnologyForSimulation, selectedVendorForSimulation, getTokenSelectList, tokenCheckBoxValue } = useSelector(state => state.simulation)
    ...
    const renderModule = (value) => {
        ...
        switch (value.value) {
            ...
            case RMIMPORT:
                return (<RMImportListing isSimulation={true} technology={technology.value} isMasterSummaryDrawer={masterSummaryDrawerState ? props.isMasterSummaryDrawer : false} apply={editTable} objectForMultipleSimulation={obj} tokenArray={token} selectionForListingMasterAPI={selectionForListingMasterAPI} changeSetLoader={changeSetLoader} changeTokenCheckBox={changeTokenCheckBox} />)
            case MACHINERATE:
