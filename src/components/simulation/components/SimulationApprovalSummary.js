    ...
    useEffect(() => {
        ...
        dispatch(getApprovalSimulatedCostingSummary(reqParams, res => {
            const { SimulationSteps, SimulatedCostingList, SimulationApprovalProcessId, Token, NumberOfCostings, IsSent, IsFinalLevelButtonShow,
                ...
                SenderReason, ImpactedMasterDataList, AmendmentDetails, Attachements, DepartmentId, TotalImpactPerQuarter, SimulationHeadId, TotalBudgetedPriceImpactPerQuarter } = res.data.Data
    ...
    const renderColumn = () => {
        ...
        if (isMultiTechnology) {
            ...
            switch (String(SimulationTechnologyId)) {
                ...
                case BOPDOMESTIC:
                case BOPIMPORT:
                    return returnExcelColumn(isTokenAPI ? finalGrid : (bopAssociation ? SIMULATIONAPPROVALSUMMARYDOWNLOADBOP : SIMULATIONAPPROVALSUMMARYDOWNLOADBOPWITHOUTCOSTING), downloadGrid.length > 0 ? downloadGrid : [])
                case MACHINERATE:
                            ...
                            <Col md="12" className="mb-2">
                                <Table responsive className="table cr-brdr-main sub-table">  {/* sub table class is alternative className which will use in future for added styles */}
                                    <thead>
                                        <tr>
                                            <th>Token No:</th>
                                            {bopAssociation && <th>Technology:</th>}
                                            ...
                                            {bopAssociation && <th>Impact/Quarter (w.r.t. Existing):</th>}
                                            {bopAssociation && <th>Impact/Quarter (w.r.t. Budgeted Price):</th>}
                                        </tr>
                                    </thead>
                                    <tbody>
                                        <tr>
                                            <td>{simulationDetail && simulationDetail.AmendmentDetails?.TokenNumber}</td>
                                            {bopAssociation && <td>{simulationDetail && simulationDetail.AmendmentDetails?.Technology}</td>}
                                            ...
                                            {bopAssociation && <td>{simulationDetail && (simulationDetail?.TotalImpactPerQuarter ? checkForDecimalAndNull(simulationDetail?.TotalImpactPerQuarter, getConfigurationKey().NoOfDecimalForPrice) : '-')}</td>}
                                            {bopAssociation && <td>{simulationDetail && (simulationDetail?.TotalBudgetedPriceImpactPerQuarter ? checkForDecimalAndNull(simulationDetail?.TotalBudgetedPriceImpactPerQuarter, getConfigurationKey().NoOfDecimalForPrice) : '-')}</td>}
                                        </tr>
                                    </tbody>
                                </Table>
                            </Col>
                                                    ...
                                                    <div className={`ag-grid-wrapper height-width-wrapper ${(costingList && costingList?.length <= 0) || noData ? "overlay-contain" : ""}`}>
                                                        ...
                                                        <div className="ag-theme-material" >
                                                            ...
                                                            >
                                                                ...
                                                                {bopAssociation && <AgGridColumn width={160} field="CostingNumber" headerName="Costing Id"></AgGridColumn>}
                                                                ...
                                                                {bopAssociation && <AgGridColumn width={150} field="ECNNumber" headerName='ECN No.' cellRenderer='ecnFormatter'></AgGridColumn>}
                                                                {bopAssociation && <AgGridColumn width={150} field="RevisionNumber" headerName='Revision No.' cellRenderer='revisionFormatter'></AgGridColumn>}
                                                                ...
                                                                {bopAssociation && <AgGridColumn width={140} field="OldPOPrice" cellRenderer='oldPOFormatter' headerName={String(SimulationTechnologyId) === EXCHNAGERATE ? 'PO Price' : "Existing PO Price"}></AgGridColumn>}

                                                                {String(SimulationTechnologyId) !== EXCHNAGERATE && bopAssociation && <AgGridColumn width={140} field="NewPOPrice" cellRenderer='newPOFormatter' headerName="Revised PO Price"></AgGridColumn>}
                                                                {String(SimulationTechnologyId) !== EXCHNAGERATE && bopAssociation && <AgGridColumn width={140} field="POVariance" headerName="Variance (w.r.t. Existing)" cellRenderer='POVarianceFormatter' ></AgGridColumn>}
                                                                {String(SimulationTechnologyId) !== EXCHNAGERATE && bopAssociation && <AgGridColumn width={140} field="BudgetedPriceVariance" headerName='Variance (w.r.t. Budgeted)' cellRenderer='impactPerQuarterFormatter'></AgGridColumn>}
                                                                ...
                                                                {(isBOPDomesticOrImport || keysForDownloadSummary.IsBoughtOutPartSimulation) && bopAssociation && <AgGridColumn width={140} field="BoughtOutPartName" headerName="Bought Out Part Name" cellRenderer='bopNameFormatter'></AgGridColumn>}
                                                                {(isBOPDomesticOrImport || keysForDownloadSummary.IsBoughtOutPartSimulation) && bopAssociation && <AgGridColumn width={140} field="BoughtOutPartNumber" headerName="Bought Out Part Number" cellRenderer='bopNumberFormatter'></AgGridColumn>}
                                                                {(isBOPDomesticOrImport || keysForDownloadSummary.IsBoughtOutPartSimulation) && <AgGridColumn width={140} field="OldNetBoughtOutPartCost" headerName={`Existing BOP ${bopAssociation ? 'Cost' : 'Rate'}`} cellRenderer='oldBOPFormatter' ></AgGridColumn>}
                                                                ...
                                                                {bopAssociation && <AgGridColumn width={140} field="SimulationCostingId" pinned="right" cellRenderer='buttonFormatter' floatingFilter={false} headerName="Actions" type="rightAligned"></AgGridColumn>}
