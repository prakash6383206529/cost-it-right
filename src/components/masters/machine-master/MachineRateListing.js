...
const MachineRateListing = (props) => {
  ...
  const [state, setState] = useState({
    ...
    floatingFilterData: {
      ...
      EffectiveDateNew: "",
      // props.isSimulation
      DepartmentName: getConfigurationKey().IsCompanyConfigureOnPlant
        ? userDepartmetList()
        : "",
      CustomerName: "",
      UOM: "",
  ...
  const { machineDatalist, allMachineDataList } = useSelector(
    (state) => state.machine
  );
  console.log(machineDatalist, allMachineDataList);
  const selectedRowForPagination = useSelector(
    (state) => state.simulation.selectedRowForPagination
  );
  ...
  useEffect(() => {
    if (machineDatalist) {
      setState((prevState) => ({
        ...prevState,
        tableData: machineDatalist,
        totalRecordCount:
          machineDatalist.length > 0 ? machineDatalist[0].TotalRecordCount : 0,
      }));
    }
  }, [machineDatalist]);

  useEffect(() => {
    const skip = (state.pageNo - 1) * state.globalTake;
    getDataList(
      "",
      0,
      "",
  ...
  const getDataList = (
    ...
    const filterData = {
      ...
      technology_id: props.isSimulation
        ? props.technology.value
        : technology_id,
      vendor_id: vendor_id,
    ...
    if (
      ...
      try {
        const res = dispatch(
          getMachineDataList(filterData, skip, take, isPagination, dataObj)
        );
        ...
        if (res) {
          ...
          setTimeout(() => {
            ...
            isReset
              ...
              : gridOptions?.api?.setFilterModel(FloatingfilterData);
            setTimeout(() => {
              setState((prevState) => ({
                ...prevState,
                warningMessage: false,
  ...
  const viewProcessGroupDetail = useCallback(
    (rowData) => {
      dispatch(
        getProcessGroupByMachineId(rowData.MachineId, (res) => {
  ...
  const onFloatingFilterChanged = useCallback(
    (value) => {
      setTimeout(() => {
      ...
      onFloatingFilterChanged(value, gridOptions, state); // COMMON FUNCTION
    },
    [machineDatalist, state.noData]
  );

  const onSearch = useCallback(() => {
    onSearch(gridOptions, state, "Machine", state.globalTake); // COMMON PAGINATION FUNCTION
  }, [gridOptions, state]);
  ...
  const closeProcessGroupDrawer = useCallback(() => {
    setState((prevState) => ({
      ...prevState,
      isOpenProcessGroupDrawer: false,
    }));
  ...
  const confirmDelete = useCallback(
    (ID) => {
      ...
      dispatch(
        deleteMachine(ID, loggedInUser, (res) => {
          if (res.data.Result === true) {
  ...
  const buttonFormatter = (props) => {
    ...
    return (
      <>
        ...
        {state.isProcessGroup && (
          <button
            className="group-process"
            type={"button"}
            title={"View Process Group"}
            onClick={() => viewProcessGroupDetail(rowData)}
          />
        )}
        {permissions.View && (
          <button
        ...
        <button
          className="Copy All Costing"
          title="Copy Machine"
          type={"button"}
          onClick={() => copyItem(cellValue)}
        />
        {isDeleteButton && (
          <button
            title="Delete"
  ...
  const costingHeadFormatter = (props) => {
    ...
    if (selectedRowForPagination?.length > 0) {
      selectedRowForPagination.map((item) => {
        if (
          item.MachineProcessRateId === props.node.data.MachineProcessRateId
        ) {
   ...
   */
  const plantsFormatter = (cell, row, enumObject, rowIndex) => {
    return cell === "" ? "-" : cell;
  };
  ...
  const returnExcelColumn = (data = [], TempData) => {
    ...
    return (
      <ExcelSheet data={temp} name={`${MachineRate}`}>
        {excelData &&
    ...
    if (selectedRows === undefined || selectedRows === null) {
      ...
      if (event.node.isSelected() === false) {
        ...
        for (let i = 0; i < selectedRowForPagination.length; i++) {
          if (selectedRowForPagination[i].MachineId === event.data.MachineId) {
  ...
  return (
    <div
      className={`ag-grid-react ${
        ...
        props.isSimulation ? "simulation-height" : ""
      }`}
    >
      <form
      ...
      >
        ...
        <Row
          className={`${
            props?.isMasterSummaryDrawer ? "" : "pt-4"
          } filter-row-large ${
            props.isSimulation || props.benchMark
              ? "simulation-filter zindex-0"
              : ""
          }`}
        >
          <Col md="3" lg="3">
              ...
              {permissions.Download && (
                <>
                  <button
                    ...
                    className={"user-btn mr5"}
                  >
                    <div className="download mr-1" title="Download"></div>
                    {/* DOWNLOAD */}
                    {`${
                      state.dataCount === 0
                        ? "All"
                  ...
                  <ExcelFile
                    filename={"Machine Rate"}
                    fileExtension={".xls"}
                    element={
                      <button
                        id={"Excel-Downloads-machine"}
        ...
        <Col>
          <div
            className={`ag-grid-wrapper height-width-wrapper ${
              (machineDatalist && machineDatalist?.length <= 0) || state.noData
                ? "overlay-contain"
                : ""
            }`}
          >
                  ...
                  <div className="d-flex pagination-button-container">
                    ...
                    {state?.pageSize?.pageSize10 && (
                      <p className="next-page-pg custom-left-arrow">
                        Page{" "}
                        <span className="text-primary">{state.pageNo}</span> of{" "}
      ...
      {state.showPopup && (
        <PopupMsgWrapper
          ...
          message={`${MESSAGES.MACHINE_DELETE_ALERT}`}
        />
      )}
      {state.showCopyPopup && (
        <PopupMsgWrapper
          isOpen={state.showCopyPopup}
          closePopUp={closePopUp}
          confirmPopup={onCopyPopupConfirm}
          message={`${MESSAGES.COPY_MACHINE_POPUP}`}
        />
      )}
      {state.isOpenProcessGroupDrawer && (
        <ProcessGroupDrawer
          anchor={"right"}
          isOpen={state.isOpenProcessGroupDrawer}
          toggleDrawer={closeProcessGroupDrawer}
          isViewFlag={true}
        />
      )}
    </div>
