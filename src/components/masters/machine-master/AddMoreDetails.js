...
class AddMoreDetails extends Component {
  ...
  */
  componentDidMount() {
    ...
    const manufactureYearDate = editDetails?.yearOfManufacturing ? new Date(`${editDetails?.yearOfManufacturing}-01-01`) : '';
    this.setState({ machineType: editDetails?.machineType, manufactureYear: manufactureYearDate })
  ...
  getMachineNumber = _.debounce((machineData) => {
    const { initialConfiguration, editDetails } = this.props;

    // Call the API with the machine data object
    if (initialConfiguration?.IsAutoGeneratedMachineNumber && editDetails && editDetails.isEditFlag === false) {
      this.props.checkAndGetMachineNumber(machineData, res => {
        let Data = res?.data?.DynamicData;
        let warningMessage = ""
        if (res?.data?.Message.includes("already exists")) {
          warningMessage = `A machine with the data you provided already exists. Its machine number is ${Data?.MachineNumber}.`
        }
        this.setState({ showMachineNoWarning: warningMessage })
        this.props.change('MachineNumber', Data?.MachineNumber)
        this.setState({ machineNumber: Data?.MachineNumber })
      })
    }
  ...
  checkAndCallGetMachineNumber = () => {
    const { fieldsObj } = this.props;
    const { selectedTechnology, selectedPlants, machineType } = this.state;

    // Check if all required fields are filled
    const hasAllRequiredFields =
      selectedTechnology && selectedTechnology.value &&
      selectedPlants && selectedPlants.value &&
      fieldsObj.MachineName &&
      fieldsObj.Specification &&
      fieldsObj.TonnageCapacity &&
    ...
    if (hasAllRequiredFields) {
      ...
      this.getMachineNumber(requestData);
    }
  }
    ...
    setTimeout(() => {
      if (nextProps.data !== this.props.data) {
        ...
        this.setState({
          ...
          // Use setTimeout to ensure the field value is updated in props before checking
          setTimeout(() => {
            this.checkAndCallGetMachineNumber();
          }, 100);
  ...
  */
  ...
  buildRequestData = (values) => {
    const {
      ...
      FuelEntryId, isDateChange, MachinePowerDetails, depreciationYearToggle, manufactureYear
    } = this.state;
    ...
    return {
      ...
      YearOfManufacturing: manufactureYear ? manufactureYear.getFullYear() : null,
                      ...
                      <Row>
                        ...
                        <Col md="3">
                          <Field
                            ...
                            className=" "
                            customClassName="withBorder"
                            onChange={this.handleTonnageCapacity}
                          />
                        </Col>
                        ...
                        <Col md="3">
                          <Field
                            ...
                            warningMessage={this.state.showMachineNoWarning}
...
function mapStateToProps(state) {
  ...
  const fieldsObj = selector(state, 'MachineCost', 'AccessoriesCost', 'InstallationCharges', 'LabourCostPerAnnum', 'LabourCostPerMonth', 'LabourCostPerShift', 'TotalCost', "LabourWorkingShift",
    ...
    'MachineFloorCRMHead', 'OtherYearlyCRMHead', 'PowerCRMHead', 'FuelCRMHead', "MachineRateConversion", "MachineRateLocalConversion", "plantCurrency", "Currency", "ExchangeSource", "Applicability", "Type", "Percentage","TonnageCapacity", "MachineName");
