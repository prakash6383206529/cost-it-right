...
*/
    ...
    */
        ...
        if (!this.props.initialConfiguration.IsAutoGeneratedCustomerCompanyCode) { // When the key is false
            ...
            this.props.checkAndGetCustomerCode(e.target.value, this.state.companyName, res => {
                ...
                if (res && res.status === 202) {
                    this.props.change('CompanyCode', res.data.Data.CustomerName);
                    this.props.change('CompanyCode', res.data.Data.CustomerCode);
                }
            ...
            this.props.checkAndGetCustomerCode(e.target.value, this.state.companyName, res => {
                ...
                if (Data?.IsExist) {
                    if (this.state.operationName) {
                        this.props.change('CompanyCode', Data.DynamicData.CustomerCode || '');
                    } else {
    ...
    checkUniqCodeByName = debounce((e) => {
        console.log('this.props.initialConfiguration.IsAutoGeneratedCustomerCode: ', this.props.initialConfiguration.IsAutoGeneratedCustomerCompanyCode);
        this.setState({ companyName: e.target.value })
            ...
            this.props.checkAndGetCustomerCode(this.state.companyCode, e.target.value, res => {
                if (res && res.data && res.data.Result === false) {
                    ...
                    this.setState({ isDisableCode: !res.data.Result }, () => {
                        this.props.change('OperaCompanyCodetionCode', res.data.Identity ? res.data.Identity : '')
                        Toaster.warning(res.data.Message);
                    })
                }
            })
