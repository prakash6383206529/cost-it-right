    ...
    switch (action.type) {
        ...
        case EXCHANGE_RATE_DATALIST:
            ...
            arr = action.payload && action.payload.filter((el) => {
                ...
                el.vendorExcel = `${el.VendorName ? el.VendorName : ''} ${el.VendorCode ? ` (${el.VendorCode})` : "-"}`
                el.customerExcel = `${el.CustomerName ? el.CustomerName : ''} ${el.CustomerCode ? ` (${el.CustomerCode})` : "-"}`
                return el
            })
            return {
                ...state,
