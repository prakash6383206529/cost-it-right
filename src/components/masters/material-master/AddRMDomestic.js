  ...
  allFieldsInfoIcon(setData) {
    ...
    if (showScrapKeys?.showCircleJali) {
      ...
      obj = {
        ...
        toolTipTextForgingScrapCostBaseCurrencyPerOldUOM: <div className='d-flex'>{labelWithUOMAndCurrency("Forging Scrap Cost", this.state.UOM?.label, initialConfiguration?.BaseCurrency)} = {labelWithUOMAndUOM("Calculated Factor", this.state.UOM?.label, this.state.ScrapRateUOM?.label)} * {labelWithUOMAndCurrency("Forging Scrap Cost", this.state.ScrapRateUOM?.label, initialConfiguration?.BaseCurrency)}</div>,
      }
      ...
      obj = {
        ...
        toolTipTextScrapCostBaseCurrencyPerOldUOM: <div className='d-flex'>{labelWithUOMAndCurrency("Scrap Cost", this.state.UOM?.label, initialConfiguration?.BaseCurrency)} = {labelWithUOMAndUOM("Calculated Factor", this.state.UOM?.label, this.state.ScrapRateUOM?.label)} * {labelWithUOMAndCurrency("Scrap Cost", this.state.ScrapRateUOM?.label, initialConfiguration?.BaseCurrency)}</div>,
      }
    }
    if (setData) {
      setTimeout(() => {
  ...
  */
  ...
  render() {
    ...
    const labelForScrapRate = () => {
      ...
      if (showScrapKeys?.showCircleJali) {
        label = labelWithUOMAndCurrency("Jali Scarp Rate", this.state.ScrapRateUOM?.label ? this.state.ScrapRateUOM?.label : 'UOM', (initialConfiguration?.BaseCurrency ? initialConfiguration?.BaseCurrency : 'Currency'))
      } else if (showScrapKeys?.showForging) {
        label = labelWithUOMAndCurrency("Forging Scarp Rate", this.state.ScrapRateUOM?.label ? this.state.ScrapRateUOM?.label : 'UOM', (initialConfiguration?.BaseCurrency ? initialConfiguration?.BaseCurrency : 'Currency'))
      } else if (showScrapKeys?.showScrap) {
                        ...
                        <Row className='UOM-label-container'>
                          ...
                          <>
                            ...
                            {showScrapKeys?.showScrap &&
                              <Col md="3">
                                {this.state.IsApplyHasDifferentUOM === true && <TooltipCustom id="scrap-rate-base-currency" width={'450px'} tooltipText={this.allFieldsInfoIcon()?.toolTipTextScrapCostBaseCurrencyPerOldUOM} />}
                                <Field
                            ...
                            {showScrapKeys?.showForging &&
                              <>
                                ...
                                <Col md="3">
                                  <Field
                                    label={labelWithUOMAndCurrency("Machining Scrap Cost", this.state.UOM?.label === undefined ? 'UOM' : this.state.UOM?.label, (initialConfiguration?.BaseCurrency ? initialConfiguration?.BaseCurrency : 'Currency'))}
                                    name={"MachiningScrap"}
                                    type="text"
                                    placeholder={isViewFlag ? '-' : "Enter"}
                            ...
                            {showScrapKeys?.showCircleJali &&
                              <>
                                ...
                                <Col md="3">
                                  <Field
                                    label={labelWithUOMAndCurrency("Circle Scrap Cost", this.state.UOM?.label === undefined ? 'UOM' : this.state.UOM?.label, (initialConfiguration?.BaseCurrency ? initialConfiguration?.BaseCurrency : 'Currency'))}
...
function mapStateToProps(state) {
  ...
  return {
    ...
    initialConfiguration, costingSpecifiTechnology, costingHead, clientSelectList, userMasterLevelAPI, rmSpecificationList
  }
}
