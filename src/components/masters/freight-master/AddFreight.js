...
class AddFreight extends Component {
            ...
            this.props.getFreightData(data.Id, res => {
                if (res && res.data && res.data.Result) {
                    ...
                    setTimeout(() => {
                        ...
                        let vendorObj = vendorListByVendorType && vendorListByVendorType.find(item => item.Value === Data.VendorId)
                        let sourceLocationObj = cityList && cityList.find(item => item.Value === Data.SourceCityId)
                        let destinationLocationObj = cityList && cityList.find(item => item.Value === Data.DestinationCityId)
                        ...
                        this.setState({
                            ...
                            TransPortMood: modeObj && modeObj !== undefined ? { label: modeObj.Text, value: modeObj.Value } : [],
                            ...
                            destinationLocation: destinationLocationObj && destinationLocationObj !== undefined ? { label: destinationLocationObj.Text, value: destinationLocationObj.Value } : [],
                            gridTable: GridArray,
                        })
                    }, 200)
                }
    ...
    */
    renderListing = (label) => {
        ...
        if (label === 'FREIGHT_MODE') {
            freightModeSelectList && freightModeSelectList.map(item => {
                if (item.Value === '0') return false;
                temp.push({ label: item.Text, value: item.Value })
            });
            return temp;
    ...
    */
    onPressLoadUnload = () => {
        this.setState({ IsLoadingUnloadingApplicable: !this.state.IsLoadingUnloadingApplicable });
    }
    ...
    */
    ...
    gridHandler = () => {
        const { FullTruckCapacity, RateCriteria, gridTable, effectiveDate } = this.state;
        ...
        const isExist = gridTable.findIndex(el => (el.CapacityId === FullTruckCapacity.value && el.RateCriteriaId === RateCriteria.value))
        if (isExist !== -1) {
    ...
    */
    updateGrid = () => {
        ...
        const isExist = skipEditedItem.findIndex(el => (el.Capacity === FullTruckCapacity.value && el.RateCriteria === RateCriteria.value))
        if (isExist !== -1) {
            toastr.warning('Already added, Please check the values.')
            return false;
        }
    ...
    */
    editGridItemDetails = (index) => {
        ...
        this.setState({
            gridEditIndex: index,
            isEditIndex: true,
            FullTruckCapacity: { label: tempData.Capacity, value: tempData.Capacity },
            RateCriteria: { label: tempData.RateCriteria, value: tempData.RateCriteria },
    ...
    */
    onSubmit = (values) => {
        const { IsVendor, TransPortMood, vendorName, IsLoadingUnloadingApplicable, sourceLocation, destinationLocation,
                            ...
                            <div className="col-md-12">
                                <div className="shadow-lgg login-formg">
                                    <div className="row">
                                        <div className="col-md-6">
                                            <div className="form-heading">
                                                <h2>{isEditFlag ? `Update Freight Details` : `Add Freight Details`}</h2>
                                            </div>
                                        </div>
                                    </div>
                                    <form
                                    ...
                                    >
                                                ...
                                                <div className="d-flex justify-space-between align-items-center inputwith-icon">
                                                    <div className="fullinput-icon">
                                                        <Field
                                                            ...
                                                            required={true}
                                                            handleChangeDescription={this.handleTransportMoodChange}
                                        ...
                                        <Row>
                                            {this.state.IsLoadingUnloadingApplicable && <Col md="3">
                                                <Field
                                                    label={``}
                                        ...
                                        <Row>
                                                ...
                                                <div className="d-flex justify-space-between align-items-center inputwith-icon">
                                                    <div className="fullinput-icon">
                                                        <Field
                                                            ...
                                                            component={searchableSelect}
                                                            placeholder={'--Select--'}
                                                            options={this.renderListing('FULL_TRUCK_CAPACITY')}
                                                            //onKeyUp={(e) => this.changeItemDesc(e)}
                                            ...
                                            <Col md="3">
                                                <Field
                                                    ...
                                                    component={searchableSelect}
                                                    placeholder={'--Select--'}
                                                    options={this.renderListing('FREIGHT_RATE_CRITERIA')}
                                                    //onKeyUp={(e) => this.changeItemDesc(e)}
...
function mapStateToProps(state) {
    ...
    const { comman, material, freight } = state;

    const { vendorListByVendorType } = material;
    const { cityList, } = comman;
    const { freightData, freightModeSelectList, freightFullTruckCapacitySelectList, freightRateCriteriaSelectList, } = freight
    ...
    return {
        cityList, vendorListByVendorType, freightModeSelectList, freightFullTruckCapacitySelectList, freightRateCriteriaSelectList,
        freightData, fieldsObj, initialValues,
    }
