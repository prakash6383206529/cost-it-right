...
function VolumeListing(props) {
  ...
  const { volumeDataList, volumeDataListForDownload } = useSelector(
    (state) => state.volume
  );
  ...
  const applyPermission = (topAndLeftMenuData) => {
    if (topAndLeftMenuData !== undefined) {
      ...
      const accessData =
        Data && Data.Pages.find((el) => el.PageName === VOLUME);
  ...
  const getTableListData = (skip = 0, take = 10, isPagination = true) => {
    ...
    dataObj.IsCustomerDataShow = reactLocalStorage.getObject(
      "cbcCostingPermission"
    );
    dispatch(
      getVolumeDataList(skip, take, isPagination, dataObj, (res) => {
  ...
  return (
    <>
      <div
        className={`ag-grid-react container-fluid blue-before-inside ${
      ...
      >
          ...
          <>
            ...
            <div
              className={`ag-grid-wrapper height-width-wrapper  ${
                (volumeDataList && volumeDataList?.length <= 0) || noData
                  ? "overlay-contain"
                  : ""
              }`}
            >
              ...
              >
                ...
                >
                  ...
                  <AgGridColumn
                    field="PartNumber"
                    headerName="Part No. (Revision No.)"
                    width={200}
                  ></AgGridColumn>
                  <AgGridColumn
                    field="PartName"
        ...
        {showPopup && (
          <PopupMsgWrapper
            ...
            message={`${MESSAGES.VOLUME_DELETE_ALERT}`}
          />
        )}
