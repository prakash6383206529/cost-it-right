        ...
        const onSubmit = debounce(handleSubmit(() => {
            ...
            if (type === 'Sender') {
                ...
                if (props?.isRFQ === true && Array.isArray(approvalDetails) && approvalDetails.length > 0) {
                    ...
                    Promise.all(approvalDetails.map(processApproval))
                        .then(() => {
                            setIsDisable(false);
                            setIsLoader(false);
                            if (IsPushDrawer) {
                    ...
                    // Original code for single approval
                    let obj = {
                        ApprovalProcessSummaryId: approvalDetails?.MasterApprovalProcessSummaryId,
                        ApprovalProcessId: approvalDetails?.ApprovalProcessId,
                        ApprovalToken: approvalDetails?.Token,
                        LoggedInUserId: loggedInUserId(),
                        SenderLevelId: levelDetails.LevelId,
                        SenderId: loggedInUserId(),
                        SenderLevel: levelDetails.Level,
            ...
            //     
            //     
            //     if( props?.isRFQ === true && approvalDetails?.length > 1){
