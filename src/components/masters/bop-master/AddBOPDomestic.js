...
import { BOP_MASTER_ID, FILE_URL, ZBC, EMPTY_GUID, SPACEBAR, VBCTypeId, CBCTypeId, ZBCTypeId, searchCount, ENTRY_TYPE_DOMESTIC, VBC_VENDOR_TYPE, BOP_VENDOR_TYPE, BOUGHT_OUT_PART_NAME } from '../../../config/constants';
  ...
  onSubmit = debounce((values) => {
    ...
    if (isEditFlag) {
      ...
      if (((files ? JSON.stringify(files) : []) === (DataToCheck.Attachements ? JSON.stringify(DataToCheck.Attachements) : [])) && ((DataToCheck.Remark ? DataToCheck.Remark : '') === (values.Remark ? values.Remark : '')) &&
        ...
        Toaster.warning(`Please change data to send ${getConfigurationKey().BOPMasterName} for approval`)
        return false
      }
      //  ELSE: CHANGE
      else {
                  ...
                  <div className="shadow-lgg login-formg">
                    <div className="row">
                      <div className="col-md-6">
                        <h1>
                          {isViewMode ? "View" : isEditFlag ? "Update" : "Add"} {getConfigurationKey().BOPMasterName} (Domestic)
                          <TourWrapper
                    ...
                    >
                      <div className="add-min-height">
                        ...
                        <Row>
                          <Col md="12">
                            <div className="left-border">{`${getConfigurationKey().BOPMasterName}:`}
                            </div>
                          </Col>
                          <Col md="3">
                            <Field
                              label={`${getConfigurationKey().BOPMasterName} Part Name`}
                            ...
                            <div className="d-flex justify-space-between align-items-center inputwith-icon">
                              <div className="fullinput-icon">
                                <Field
                                  name="BOPCategory"
                                  type="text"
                                  label={`${getConfigurationKey().BOPMasterName} Category`}
                          ...
                          <Col md="3">
                            <Field
                              label={`${getConfigurationKey().BOPMasterName} Part No`}
                          ...
                          {initialConfiguration?.IsBoughtOutPartCostingConfigured && costingTypeId === VBCTypeId &&
                            <>
                              <Col md="3" className='d-flex align-items-center'>
                                ...
                                >
                                  Detailed ${getConfigurationKey().BOPMasterName}
                                  <input
                                    type="checkbox"
