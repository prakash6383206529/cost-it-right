...
export const checkRM_Process_OperationConfigurable = (excelData, typeId = null) => {
    ...
    return excelData.filter((el) => {
        ...
        if (showDetails) {
            if (calcType === FUELANDPOWER_CALCULATION_TYPE.Custom) {
                hiddenFields = customHidden;
            } else if (calcType === FUELANDPOWER_CALCULATION_TYPE.Standard) {
                hiddenFields = standardHidden;
            }
        } else if (typeId === ZBCADDMORE) {
            if (calcType === FUELANDPOWER_CALCULATION_TYPE.Custom) {
                hiddenFields = customHidden;
            } else if (calcType === FUELANDPOWER_CALCULATION_TYPE.Standard) {
                hiddenFields = standardHidden;
        ...
        if (hiddenFields.includes(el.value)) {
            return false;
        }
    ...
    renderZBCSwitch = (master) => {
        ...
        switch (master) {
            ...
            case ZBCADDMORE:
                ...
                return this.returnExcelColumn(checkVendorPlantConfig(checkRM_Process_OperationConfigurable(localizedMHRMoreHeaders, ZBCADDMORE), ZBCADDMORE), MHRMoreZBCTempData);
            case VBCADDMORE:
                const localizedMHRMoreHeadersVBC = this.localizeHeaders(MHRMoreVBC);
                return this.returnExcelColumn(checkVendorPlantConfig(checkRM_Process_OperationConfigurable(localizedMHRMoreHeadersVBC, VBCADDMORE), VBCADDMORE), MHRMoreVBCTempData);
            case CBCADDMORE:
                const localizedMHRMoreHeadersCBC = this.localizeHeaders(MHRMoreCBC);
                return this.returnExcelColumn(checkVendorPlantConfig(checkRM_Process_OperationConfigurable(localizedMHRMoreHeadersCBC, CBCADDMORE), CBCADDMORE), MHRMoreCBCTempData);
            ...
            return (
                <>
                    ...
                    {bopType === DETAILED_BOP &&
                        <ExcelFile filename={`Detailed ${fileName} VBC`} fileExtension={'.xls'} element={<button type="button" className={'btn btn-primary pull-right w-100'}><div class="download"></div> Download</button>}>
