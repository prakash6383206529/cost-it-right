...
export const checkVendorPlantConfig = (excelData) => {
    return excelData.filter((el) => {
        const { costingTypeId } = this.props
        ...
        if (costingTypeId === CBCTypeId && getConfigurationKey().IsCBCApplicableOnPlant === false) {
            if (el.value === 'DestinationPlant') return false;
            if (el.value === 'DestinationPlantCode') return false;
        }
        return true;
...
class Downloadxls extends React.Component {
    ...
    */
    ...
    renderCBCSwitch = (master) => {
        switch (master) {
            case 'RMDomestic':
                return this.returnExcelColumn(checkVendorPlantConfig(RMDomesticCBC), RMDomesticCBCTempData);
            case 'RMImport':
                return this.returnExcelColumn(checkVendorPlantConfig(RMImportCBC), RMImportCBCTempData);
            case 'Operation':
                return this.returnExcelColumn(checkLabourRateConfigure(CBCOperation), CBCOperationTempData);
            case 'Machine':
                return this.returnExcelColumn(checkVendorPlantConfig(MachineCBC), MachineCBCTempData);
            case 'BOPDomestic':
                return this.returnExcelColumn(checkVendorPlantConfig(BOP_CBC_DOMESTIC), BOP_CBC_DOMESTIC_TempData);
            case 'BOPImport':
                return this.returnExcelColumn(checkVendorPlantConfig(BOP_CBC_IMPORT), BOP_CBC_IMPORT_TempData);
            case 'ActualVolume':
                return this.returnExcelColumn(checkVendorPlantConfig(VOLUME_ACTUAL_CBC), VOLUME_ACTUAL_CBC_TEMPDATA);
            case 'BudgetedVolume':
                return this.returnExcelColumn(checkVendorPlantConfig(VOLUME_BUDGETED_CBC), VOLUME_BUDGETED_CBC_TEMPDATA);
            case 'InterestRate':
    ...
    */
    ...
    render() {
        ...
        if (isFailedFlag && (costingTypeId === ZBCTypeId || costingTypeId === ZBCADDMORE) && (fileName === 'RMDomestic' || fileName === 'RMImport' || fileName === 'Operation' || fileName === 'Machine' || fileName === 'BOPDomestic' || fileName === 'BOPImport' || fileName === 'ActualVolume' || fileName === 'BudgetedVolume' || fileName === 'InterestRate')) {
        ...
        if (isFailedFlag && costingTypeId === VBCTypeId && (fileName === 'RMDomestic' || fileName === 'RMImport' || fileName === 'Operation' || fileName === 'Machine' || fileName === 'BOPDomestic' || fileName === 'BOPImport' || fileName === 'ActualVolume' || fileName === 'BudgetedVolume' || fileName === 'InterestRate')) {
            return (
